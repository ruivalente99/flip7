...
export function WinOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 animate-in fade-in duration-300">
      <div className="text-center space-y-6 p-8 rounded-2xl bg-slate-900 border border-slate-700 max-w-sm w-full mx-4 animate-in zoom-in-75 duration-500">
        ...
        <div>
          ...
          <p className="text-slate-400 mt-1 text-lg">{winner?.totalScore} points</p>
        </div>
          ...
          {[...localGame.players]
            ...
            .map((p, i) => (
              <div key={p.id} className="flex justify-between">
                ...
                <span className="font-bold text-slate-300">{p.totalScore}</span>
              </div>
            ))}
        </div>
        <Button
          onClick={handleClose}
          className="w-full bg-indigo-600 hover:bg-indigo-500 font-bold text-lg py-5"
        >
          Back to Menu
        </Button>
      </div>
